...
# Global variable to store environment LLM config for FakeUser
_env_llm_config: LLMConfig = None
...
class FakeUser:
    def __init__(self, runtime: Runtime, llm_config: LLMConfig):
        ...
        # Create OpenAI client with config from llm_config
        # For Ollama, api_key can be any string (e.g., "ollama")
        api_key = llm_config.api_key if llm_config.api_key else "ollama"
        if not llm_config.api_key or llm_config.api_key in ["ollama", "none", "local"]:
            logger.warning(f"⚠️  No API key provided for environment LLM. Using placeholder for local Ollama model.")
...
def codeact_user_response(state: State) -> str:
    ...
    global fake_user, runtime, _env_llm_config
    if 'fake_user' not in globals():
        if _env_llm_config is None:
            raise ValueError("Environment LLM config not initialized. Call init_task_env first.")
        fake_user = FakeUser(runtime, _env_llm_config)
...
def init_task_env(runtime: Runtime, hostname: str, env_llm_config: LLMConfig, task_path: str):
    # Set global env_llm_config for FakeUser to use
    global _env_llm_config
    _env_llm_config = env_llm_config
    ...
    # Handle api_key for Ollama (doesn't need real key)
    api_key = env_llm_config.api_key if env_llm_config.api_key else "ollama"
...
if __name__ == '__main__':
    ...
    if agent_llm_config.api_key is None or agent_llm_config.api_key in ["ollama", "none", "local"]:
        logger.warning(f'⚠️  No valid API key for agent LLM. If using Ollama, this is expected. Otherwise, add your API key to config.toml')
    ...
    if env_llm_config.api_key is None or env_llm_config.api_key in ["ollama", "none", "local"]:
        logger.warning(f'⚠️  No valid API key for environment LLM. If using Ollama, this is expected. Otherwise, add your API key to config.toml')
